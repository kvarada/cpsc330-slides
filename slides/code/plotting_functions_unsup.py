...
def plot_X_dendrogram(X, linkage_array, font_size=14, label_n_clusters=False, title='Dendrogram'): 
    ...
    if label_n_clusters: 
        ...
        axes[1].plot(bounds, [4.2, 4.2], "--", c="k")
        axes[1].plot(bounds, [2, 2], "--", c="k")    
        #axes[1].plot(bounds, [4, 4], "--", c="k")
        #axes[1].plot(bounds, [2, 2], "--", c="k")        
        #axes[1].plot(bounds, [1.3, 1.3], "--", c="k")            
        #axes[1].text(bounds[1], 7.25, " one cluster", va="center", fontdict={"size": 15})
        axes[1].text(bounds[1], 4.2, " two clusters", va="center", fontdict={"size": 15})
        axes[1].text(bounds[1], 2, " three clusters", va="center", fontdict={"size": 15})    
